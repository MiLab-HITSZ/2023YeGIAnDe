...
class UserSingleStep(torch.nn.Module):
    ...
    def compute_local_updates(self, server_payload, custom_data=None, using_L1loss=False, using_L2loss=False, using_feature_mix=False, secure_input=False, apply_noise=False, apply_prune=False):
        ...
        if custom_data is None:
            ...
            data = custom_data
            for key in data:
                try: data[key] = data[key].to(self.setup["device"])
                except: continue
        ...
        def _compute_batch_gradient(data, using_L1loss=False, using_L2loss=False, using_feature_mix=False, secure_input=False):
            data[self.data_key] = (
            ...
            if using_feature_mix:
                print('---------------using feature mixing loss---------------')
                data['inputs'] = data['inputs'][0:2]
                outputs = self.model.mix_feature_forward(**data)
                loss = self.loss(outputs, data["labels"])
            elif secure_input:
                print('---------------using secure input---------------')
                inputs = data['inputs']
                labels = data["labels"]
                n_data = inputs.shape[0]
                lam = 0.8
                mix_result = -lam * inputs
                idx = torch.arange(1,n_data+1)
                idx[-1] = 0
                tmp = inputs[idx]
                mix_result = mix_result + (1-lam) * tmp
                data['inputs'] = torch.cat([inputs, mix_result], 0)
                data["labels"] = torch.cat([labels, labels], 0)
                outputs = self.model(**data)
                loss = self.loss(outputs, data["labels"])
            else:
                outputs = self.model(**data)
                # tmpt = torch.nn.functional.softmax(outputs, dim=1).max(dim=1)
                loss = self.loss(outputs, data["labels"])
                regular_loss = torch.zeros_like(loss)
                if using_L1loss:
                    print('---------------using L1 loss---------------')
                    for param in self.model.parameters():
                        regular_loss += torch.norm(param, p=1)
                    loss += 0.1 * regular_loss
                if using_L2loss:
                    print('---------------using L2 loss---------------')
                    for param in self.model.parameters():
                        regular_loss += torch.norm(param, p=2)
                    loss += 0.1 * regular_loss
        ...
        if self.clip_value > 0:  # Compute per-example gradients and clip them in this case
            ...
            for data_idx in range(B):
                ...
                per_example_grads = _compute_batch_gradient(data_point, using_L1loss=using_L1loss, using_L2loss=using_L2loss, using_feature_mix=using_feature_mix, secure_input=secure_input)
            ...
            shared_grads = _compute_batch_gradient(data, using_L1loss=using_L1loss, using_L2loss=using_L2loss, using_feature_mix=using_feature_mix, secure_input=secure_input)
        self._apply_differential_noise(shared_grads)
        if apply_noise:
            print('---------apply noise-----------')
            self._apply_additive_noise(shared_grads)
        if apply_prune:
            print('---------apply pruning-----------')
            self._apply_pruning(shared_grads)
    ...
    def _apply_additive_noise(self, grads):
        loc = torch.as_tensor(0.0, **self.setup)
        for grad in grads:
            scale = torch.max(torch.abs(grad))*0.5
            generator = torch.distributions.laplace.Laplace(loc=loc, scale=scale)
            grad += generator.sample(grad.shape)

    def _apply_pruning(self, grads):
        prun_rate = 0.99
        for grad in grads:
            n = int(prun_rate*grad.numel())
            tmp_grad = torch.abs(grad)
            sort_data = torch.sort(tmp_grad.flatten())[0]
            cut = sort_data[n-1]
            mask = tmp_grad > cut
            grad *= mask
