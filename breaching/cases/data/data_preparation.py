...
def construct_dataloader(cfg_data, cfg_impl, user_idx=0, return_full_dataset=False):
    ...
    if cfg_impl.shuffle and not cfg_impl.fix_generator:
        data_sampler = torch.utils.data.RandomSampler(dataset, replacement=cfg_impl.sample_with_replacement)
    elif cfg_impl.shuffle and cfg_impl.fix_generator:
        generator = torch.Generator()
        generator.manual_seed(cfg_impl.generator_seed)
        data_sampler = torch.utils.data.RandomSampler(dataset, replacement=cfg_impl.sample_with_replacement, generator=generator)
    else:
