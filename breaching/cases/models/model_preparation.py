    ...
    if "ImageNet" in cfg_data.name:
        try:
            ...
            if "resnet" in cfg_model.lower():
                ...
                # replace_ReLU2ID(model)
                replace_ReLU2ELU(model)
                # replace_ReLU2Sigmoid(model)
                # replace_ReLU2LeakyReLU(model)
                # rand_set_batchnorm_para(model)
                # replace_ReLU2Tanh(model)
            elif 'vit' in cfg_model.lower():
                model.heads[0] = torch.nn.Linear(in_features=768, out_features=classes)
                # replace_ReLU2LeakyReLU(model)
            elif 'mobilenet' in cfg_model.lower():
                # replace_ReLU2ELU(model)
                remove_dropout(model)
                model.classifier[1] = torch.nn.Linear(in_features=1280, out_features=classes, bias=True)
            elif 'alexnet' in cfg_model.lower():
                # fc = torch.nn.Linear(model.classifier.in_features, classes)
                # model.classifier = fc
                # replace_ReLU2Sigmoid(model)
                replace_ReLU2ELU(model)
                remove_dropout(model)
