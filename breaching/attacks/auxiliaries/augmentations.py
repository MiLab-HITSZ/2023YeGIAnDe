...
class GaussianBlur(torch.nn.Module):
    def __init__(self, radius=0, tiny_std=0):
        super(GaussianBlur, self).__init__()
        self.trans1 = transforms.GaussianBlur(radius, tiny_std)

    def forward(self, inputs):
        return self.trans1(inputs)


class ContrastJitter(torch.nn.Module):
    def __init__(self, min, max):
        super(ContrastJitter, self).__init__()
        self.trans = transforms.ColorJitter(contrast=(min, max))

    def forward(self, input):
        return self.trans(input)


class BrightJitter(torch.nn.Module):
    def __init__(self, min, max):
        super(BrightJitter, self).__init__()
        self.trans = transforms.ColorJitter(brightness=(min, max))

    def forward(self, input):
        return self.trans(input)


class SatureJitter(torch.nn.Module):
    def __init__(self, sat_ratio=1):
        super(SatureJitter, self).__init__()
        self.trans = transforms.ColorJitter(saturation=(sat_ratio, sat_ratio))

    def _sature_input(self, tensor):
        n = tensor.shape[0]
        tmp = torch.zeros_like(tensor)
        for i in range(n):
            tmp[i] = self.trans(tensor[i])
        return tmp

    def forward(self, input):
        return self._sature_input(input)
...
augmentation_lookup = dict(
    ...
    brightjitter=BrightJitter,
    saturejitter=SatureJitter,
)
