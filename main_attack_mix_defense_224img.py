...
# @File       : main_attack_mix_defense_224img.py
...
if __name__ == '__main__':
    ...
    cfg = breaching.get_config(overrides=["case=12_custom_imagenet", "attack=our_vgg13_dyna_untrain"])
    ...
    cus_data = CustomData(data_dir='custom_data/1_img/', dataset_name='ImageNet', case='12_custom_imagenet', mix=cfg.case.data.mix, only_mix=only_mix)
    shared_data, true_user_data = user.compute_local_updates(server_payload, custom_data=cus_data.process_data(sec_input4=sercure_input), apply_noise=apply_noise, apply_prune=apply_prune)
